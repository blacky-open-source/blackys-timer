...
public OnStylesLoaded()
{
	...
	for(new Type; Type < MAX_TYPES; Type++)
	{
		...
		for(new Style; Style < g_TotalStyles; Style++)
		{
			if(g_StyleConfig[Style][Enabled] && g_StyleConfig[Style][AllowType][Type])
			{
...
public Action:SM_Style(client, args)
{
	...
	for(new Style; Style < g_TotalStyles; Style++)
	{
		if(g_StyleConfig[Style][Enabled] && g_StyleConfig[Style][AllowType][TIMER_MAIN])
		{
			GetStyleName(Style, sStyle, sizeof(sStyle));
...
public Action:SM_BStyle(client, args)
{
	...
	for(new Style; Style < g_TotalStyles; Style++)
	{
		if(g_StyleConfig[Style][Enabled] && g_StyleConfig[Style][AllowType][TIMER_BONUS])
		{
			GetStyleName(Style, sStyle, sizeof(sStyle));
...
public Action:SM_SetStyle(client, args)
{
	...
	for(new Style; Style < g_TotalStyles; Style++)
	{
		if(g_StyleConfig[Style][Enabled] && g_StyleConfig[Style][AllowType][TIMER_MAIN])
		{
			...
			{
				g_Style[client][TIMER_MAIN] = Style;
				StopTimer(client);
...
public Action:SM_SetBonusStyle(client, args)
{
	...
	for(new Style; Style < g_TotalStyles; Style++)
	{
		if(g_StyleConfig[Style][Enabled] && g_StyleConfig[Style][AllowType][TIMER_BONUS])
		{
			...
			if(StrEqual(sCommand, sStyle))
			{
				g_Style[client][TIMER_BONUS] = Style;
				StopTimer(client);
...
public Menu_Style(Handle:menu, MenuAction:action, client, param2)
{
	if(action == MenuAction_Select)
	{
		...
		if(StrContains(info, ";") != -1)
		{
			...
			StopTimer(client);
			new Type = StringToInt(sInfoExplode[0]);
			g_Style[client][Type] = StringToInt(sInfoExplode[1]);
...
public Action:OnTimerStart_Pre(client, Type, Style)
{
	...
	if(!g_StyleConfig[Style][AllowType][Type] || !g_StyleConfig[Style][Enabled])
	{
		return Plugin_Handled;
	}
...
{
	return g_StyleConfig[GetNativeCell(1)][Enabled];
}
